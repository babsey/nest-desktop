...
export class NESTConnectionGraph extends ConnectionGraph {
  ...
  render(): void {
    ...
    connections
      .style("color", (c: NESTConnection | unknown) => {
        if (!c.source) return;
    ...
    connections.each((connection: NESTConnection, idx: number, elements) => {
  ...
  update(): void {
    ...
    const connections = this._networkGraph.selector
      ...
      .data(this.network.connections.allConnections, (c: NESTConnection | unknown) => c.uuid);
