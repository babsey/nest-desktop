...
export class ConnectionGraph extends BaseObj {
  ...
  render(): void {
    ...
    connections
      .style("color", (c: TConnection | unknown) => {
        if (!c.source) return;
    ...
    connections.each((connection: TConnection, idx: number, elements: unknown[]) => {
  ...
  update(): void {
    ...
    const connections = this._networkGraph.selector
      ...
      .data(this.networkGraph.network.connections.allConnections, (c: TConnection | unknown) => c.uuid);
